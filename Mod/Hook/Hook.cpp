...
auto Hook::init() -> void
{
	...
	if (MH_EnableHook((LPVOID)playerkb) != MH_OK) {
	...
	if (MH_EnableHook((LPVOID)clientInstanceTick) != MH_OK) {
	...
	if (MH_EnableHook((LPVOID)is_ShowCoordinatesTick) != MH_OK) {
	...
	if (MH_EnableHook((LPVOID)getHungerValAddressTick) != MH_OK) {
	...
	if (MH_EnableHook((LPVOID)destroyBlockingAddress) != MH_OK) {
	...
	if (MH_EnableHook((LPVOID)noFallDamage_tick) != MH_OK) {
