...
void RenderBlockBox(vec3_ti bPos, vec3_t lPos,ImColor boxColor, float linesize)
{
	// 获取屏幕宽高
	RECT rect{};
	...
	vec2_t point[8]{};
	for (int i = 0; i < 8; i++) {
		if (refdef->OWorldToScreen(lPos, { bPos.x + offset[i].x, bPos.y + offset[i].y, bPos.z + +offset[i].z }, out, fov, { rectwidth,rectheight })) {
			point[i] = out;
		}
		else {
			point[i] = vec2_t();
